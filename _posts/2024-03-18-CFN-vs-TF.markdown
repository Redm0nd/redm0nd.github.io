...
[checkov](https://github.com/bridgecrewio/checkov) is also useful for both CFN and TF and is free _for now_. 


### Programming and Dev Experience 

The capabilities of HCL is where Terraform excels in comparison to CloudFormation. When using both these tools at scale you begin to face issues at scale. 

IaC generator, CDK didn't use much, CDK bootstrap multi-account is difficult. tags with Terraform
...
Having said that, with the recent improvements CloudFormation has made and the changes in licencing and uncertainty from Terraform. I would have to crown the IaC winer in my opinion as ... CloudFormation. 